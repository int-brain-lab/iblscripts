...
class TestEphysSyncRegisterRaw(base.IntegrationTest):
    def setUp(self) -> None:
        ...
        self.sync_path = self.session_path.joinpath('raw_ephys_data')
        self.sync_path.mkdir(parents=True)
        ...
        self.sync_path.joinpath('_spikeglx_ephysData_g0_t0.nidq.wiring.json').touch()
        nidq_meta = self.sync_path.joinpath(self.meta_path.name.replace('ap', 'nidq'))
        shutil.copy(self.meta_path, nidq_meta)

    def test_compress(self):
        nidq_file = self.sync_path.joinpath(self.file_path.name.replace('ap', 'nidq'))
        shutil.copy(self.file_path, nidq_file)
    ...
    def test_already_compressed(self):
        nidq_file = self.sync_path.joinpath(self.cfile_path.name.replace('ap', 'nidq'))
        shutil.copy(self.cfile_path, nidq_file)
        ch_file = self.sync_path.joinpath(self.ch_path.name.replace('ap', 'nidq'))
        shutil.copy(self.ch_path, ch_file)
        task = EphysSyncRegisterRaw(self.session_path, sync_collection='raw_ephys_data')
        status = task.run()
        assert status == 0
        self.check_files(task)

    def test_regitser(self):
        ch_file = self.sync_path.joinpath(self.ch_path.name.replace('ap', 'nidq'))
        shutil.copy(self.ch_path, ch_file)
        task = EphysSyncRegisterRaw(self.session_path, sync_collection='raw_ephys_data')
        status = task.run()
...
class TestEphysCompressNP21(base.IntegrationTest):
    ...
    def setUp(self) -> None:
        self.data_path = self.data_path.joinpath('ephys', 'ephys_np2', 'raw_ephys_data', 'probe00')
        self.temp_dir = Path(tempfile.TemporaryDirectory().name)
        self.session_path = self.temp_dir.joinpath('saturn', '2054-07-13', '001')
        self.probe = 'probe00'
        self.probe_path = self.session_path.joinpath('raw_ephys_data', self.probe)
        self.probe_path.mkdir(parents=True)
...
class TestEphysCompressNP24(base.IntegrationTest):
    ...
    def setUp(self) -> None:
        self.data_path = self.data_path.joinpath('ephys', 'ephys_np2', 'raw_ephys_data', 'probe00')
        self.temp_dir = Path(tempfile.TemporaryDirectory().name)
        self.session_path = self.temp_dir.joinpath('venus', '2054-07-13', '001')
        self.probe = 'probe00'
        self.probe_path = self.session_path.joinpath('raw_ephys_data', self.probe)
        self.probe_path.mkdir(parents=True)
...
class TestEphysPulses(base.IntegrationTest):
    pass
    # TODO need to test when we have 4 probes that it extracts the sync for all of them
