...
TODO:
    ...
    - on transfer, read in yml file found on server, integrate FP settings, overwrite server yml file
...
FP_REMOTE_SUBJECTS_FOLDER_TEST = None  # used for testing transfer function
if os.name == "nt":  # check on OS platform
    FP_LOCAL_DATA_FOLDER = "D:\\ibl_fp_data"  # folder that will contain the fp data output from bonsai and daq
    FP_LOCAL_DATA_FOLDER_BKUP = "D:\\ibl_fp_data_bkup\\Subjects"  # folder mirroring server dir structure
    FP_LOCAL_QUEUED_SUBJECTS_FOLDER = "C:\\Temp\\ibl_fp_data_queued\\Subjects"  # temp folder created for queued items
    try:  # to create local queue data folder
        os.makedirs(FP_LOCAL_DATA_FOLDER_BKUP, exist_ok=True)
        os.makedirs(FP_LOCAL_QUEUED_SUBJECTS_FOLDER, exist_ok=True)
    except OSError:
        raise
else:
    ...
    FP_LOCAL_DATA_FOLDER = TEMP_DIR.name + "/local/fp_data"
    FP_LOCAL_DATA_FOLDER_BKUP = TEMP_DIR.name + "/local/Subjects"
    FP_REMOTE_SUBJECTS_FOLDER_TEST = TEMP_DIR.name + "/remote/Subjects"
    FP_LOCAL_QUEUED_SUBJECTS_FOLDER = TEMP_DIR.name + "/queued/Subjects"
    try:
        os.makedirs(FP_LOCAL_DATA_FOLDER, exist_ok=True)
        os.makedirs(FP_LOCAL_DATA_FOLDER_BKUP, exist_ok=True)
        os.makedirs(FP_REMOTE_SUBJECTS_FOLDER_TEST, exist_ok=True)
        os.makedirs(FP_LOCAL_QUEUED_SUBJECTS_FOLDER, exist_ok=True)
    except OSError:
        raise
    print(f"Not a Windows OS, will only create temp files\nlocal subject dir: {FP_LOCAL_DATA_FOLDER}\nremote subject dir: "
          f"{FP_REMOTE_SUBJECTS_FOLDER_TEST}\nqueued subject dir: {FP_LOCAL_QUEUED_SUBJECTS_FOLDER}")

# TODO: Remove TEST once ready for production, create proper tests
# -----------------------------------------------------------------------------
TEST_DATA_LOC = "/home/michele/Downloads/FP_Extraction_Prototypes/rigs_data/photometry/2022-09-06/"
TEST_LOCAL_DATE_LOC = Path(FP_LOCAL_DATA_FOLDER) / Path(str(date.today()))
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def scan_local_date_folder(self):
        ...
        "fp_config_files"
        """
        date_folder = Path(FP_LOCAL_DATA_FOLDER) / Path(self.date_edit.text())
    ...
    def remove_old_sessions(self):
        ...
        if self.confirm_box.exec_() == 1:  # OK button pressed
            ...
            subject_list = os.listdir(FP_LOCAL_DATA_FOLDER)
            six_month_old_date = datetime.now() - relativedelta(months=6)

            for subject in subject_list:
                subject = str(subject)
                # get list of all date directories for a subject
                date_list = os.listdir(Path(FP_LOCAL_DATA_FOLDER).joinpath(subject))
                ...
                for date_dir in date_list:
                    ...
                    if date_dir_as_datetime < six_month_old_date:
                        dir_to_remove = Path(FP_LOCAL_DATA_FOLDER).joinpath(subject, date_dir)
    ...
    def transfer_items_to_server(self):
        ...
        if FP_REMOTE_SUBJECTS_FOLDER_TEST:  # if var is set, we should be in testing mode
            remote_folder = Path(FP_REMOTE_SUBJECTS_FOLDER_TEST)
        else:
            remote_folder = Path(self.server_data_path.text())

        try:  # Copy items from queue_data_path to local_bkup_data_path for every item in queue
            [shutil.copytree(item["queue_data_path"], item["local_bkup_data_path"], dirs_exist_ok=True) for item in self.items_to_transfer]
        except shutil.Error:
            raise
        ...
        for item in self.items_to_transfer:
            ...
            transfer_success = rsync_paths(item["local_bkup_data_path"], remote_data_path)
    ...
    def add_item_to_queue(self):
        ...
        local_bkup_data_path = Path(
            Path(FP_LOCAL_DATA_FOLDER_BKUP) /
            self.subject_combo_box.currentText() /
            self.date_edit.text() /
            self.session_number.text() /
            "raw_photometry_data")
        queue_data_path = Path(
            Path(FP_LOCAL_QUEUED_SUBJECTS_FOLDER) /
            self.subject_combo_box.currentText() /
            self.date_edit.text() /
            self.session_number.text() /
            "raw_photometry_data")
        ...
        for item in self.items_to_transfer:
            if queue_data_path == Path(item["queue_data_path"]):
        ...
        dst_fp_data_file = queue_data_path / "_neurophotometrics_fpData.raw.pqt"
        dst_daq_data_file = queue_data_path / "_mcc_DAQdata.raw.tdms"
        ...
        item = {
            ...
            "queue_data_path": str(queue_data_path),
            "local_bkup_data_path": str(local_bkup_data_path),
        ...
        try:  # to perform OS write operations for queue_data_path structure and create data files
            os.makedirs(queue_data_path, exist_ok=True)
        ...
        # Validate transfers are ready, enable transfer button
    ...
    def stringify_latest_item(self):
        ...
        if item["patch_cord_selection_01"] != "":
            si += "Patch Cord Selection 01: " + item["patch_cord_selection_01"] + "\n"
        if item["patch_cord_selection_02"] != "":
            si += "Patch Cord Selection 02: " + item["patch_cord_selection_02"] + "\n"
        if item["patch_cord_selection_03"] != "":
            si += "Patch Cord Selection 03: " + item["patch_cord_selection_03"] + "\n"
        # ROI Selectors
        if item["roi_selection_01"] != "":
            si += "ROI Selection 01: " + item["roi_selection_01"] + "\n"
        if item["roi_selection_02"] != "":
            si += "ROI Selection 02: " + item["roi_selection_02"] + "\n"
        if item["roi_selection_03"] != "":
            si += "ROI Selection 03: " + item["roi_selection_03"] + "\n"
        # Brain Areas
        if item["brain_area_01"] != "":
            si += "Brain Area 01: " + item["brain_area_01"] + "\n"
        if item["brain_area_02"] != "":
            si += "Brain Area 02: " + item["brain_area_02"] + "\n"
        if item["brain_area_03"] != "":
            si += "Brain Area 03: " + item["brain_area_03"] + "\n"
        # Notes
        if item["notes_01"] != "":
            si += "Note 01: " + item["notes_01"] + "\n"
        if item["notes_02"] != "":
            si += "Note 02: " + item["notes_02"] + "\n"
        if item["notes_03"] != "":
            si += "Note 03: " + item["notes_03"] + "\n"
        # Run Selector
    ...
    def reset_form(self):
        ...
        # Cleanup local queue_data_path files, empty self.items_to_transfer list and self.available_data_files
        for item in self.items_to_transfer:
            if item["queue_data_path"]:
                print(f"Deleting {Path(item['queue_data_path']).parent.parent.parent}")
                try:
                    shutil.rmtree(Path(item["queue_data_path"]).parent.parent.parent)
                except FileNotFoundError:
                    print(f"{Path(item['queue_data_path']).parent.parent.parent} was not found. Directory was likely already deleted.")
        self.items_to_transfer = []
        self.runs_available = []
        self.available_data_files = {}
    ...
    def populate_default_qsetting_values(self):
        ...
        if not self.settings.value("local_data_path") or os.name != "nt":  # os name check for testing on linux with temp dirs
            self.settings.setValue("local_data_path", FP_LOCAL_DATA_FOLDER)
