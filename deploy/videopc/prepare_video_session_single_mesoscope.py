...
def main(mouse: str, training_session: bool = False, new: bool = False) -> None:
    ...
    SETUP_FILE = BONSAI_WORKFLOWS_PATH / "SingleMesoscopeRig_SetupCameras.bonsai"
    RECORD_FILE = BONSAI_WORKFLOWS_PATH / "SingleMesoscopeRig_SaveVideo_EphysTasks.bonsai"
    if training_session:
    ...
    subprocess.call([str(BONSAI), str(SETUP_FILE), start, noboot, leftidx])
    # Force trigger mode on all cams
