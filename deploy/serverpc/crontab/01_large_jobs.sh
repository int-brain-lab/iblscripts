...
source "$HOME/Documents/PYTHON/envs/dlcenv/bin/activate"
...
while true; do
  ...
  if  (( $elapsed > 21600 )); then
    ...
    if [ -d "$HOME/Documents/PYTHON/envs/suite2p" ]; then
  ...
  if [ -d "$HOME/Documents/PYTHON/envs/suite2p" ]; then
    source "$HOME/Documents/PYTHON/envs/suit2p/bin/activate"
