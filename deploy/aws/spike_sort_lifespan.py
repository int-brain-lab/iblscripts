...
if __name__ == "__main__":
    ...
    complete = len(list(ssl.session_path.joinpath(f'alf/{ssl.pname}/iblsorter').glob('waveforms*'))) >= 4
    if complete:
        print(eid, ssl.pname, 'complete - skip')
        exit()
    else:
        print(eid, ssl.pname)
    if args.dry_run:
    ...
    sync_job.run()
    ssjob = SpikeSorting(session_path, one=one, pname=ssl.pname,
